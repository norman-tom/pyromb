...
class HydroConfig:
    ...
    @classmethod
    def default_config(cls, base_path: str = './output/2024/conveyance/') -> 'HydroConfig':
...
class HydroFileLoader:
    ...
    def load_temporal_pattern_data(self, aep: str, duration: str, 
    ...
    def load_all_scenarios(self) -> Dict[str, List[pd.DataFrame]]:
        ...
        for aep_value in self.config.aep_values:
            ...
            for duration in self.config.durations:
                df = self.load_temporal_pattern_data(aep_value, duration)
...
class HydroAnalyzer:
    ...
    def __init__(self, data: Dict[str, List[pd.DataFrame]], config: HydroConfig):
        self.data = data
        self.config = config
        self.results_cache = {}  # Store calculated results for reuse
    ...
    def calculate_peak_flows(self, aep: str) -> pd.DataFrame:
        ...
        to_join = [self.data[aep][i].max(axis=0).drop('time') for i in range(len(durations))]
    ...
    def find_critical_scenario(self, aep: str, time_threshold: Optional[float] = None) -> Dict[str, Any]:
        ...
        duration_format = f"du{critical_duration.replace('.', '_')}hour"
...
class HydroVisualizer:
    ...
    def plot_boxplot(self, peak_flows: pd.DataFrame, aep: str, climate_year: str, title: str,) -> plt.Figure:
        """
...
class HydroClient:
    ...
    def __init__(self, config: Optional[HydroConfig] = None):
        """
        ...
        """
        self.config = config or HydroConfig.default_config()
        self.loader = HydroFileLoader(self.config)
        self.data = None
        self.analyzer = None
        self.visualizer = HydroVisualizer(self.config)
    ...
    def load_data(self) -> Dict[str, List[pd.DataFrame]]:
        ...
        self.analyzer = HydroAnalyzer(self.data, self.config)
        return self.data
    ...
    def plot_duration_boxplot(self, aep: str, climate_year: str, title: str) -> plt.Figure:
        """
    ...
    def plot_critical_hydrograph(self, 
                                ...
                                climate_year: str,
                                title: str,
                                time_threshold: Optional[float] = None) -> Tuple[plt.Figure, Dict[str, Any]]:
        """
