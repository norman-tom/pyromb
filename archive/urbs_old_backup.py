...
class UrbsVectorWriter():
    """
    Writes URBS .vec file commands following the proper URBS text-based command structure.
    """
    ...
    def step(self, traveller: Traveller) -> None:
        """ 
        Calculate action to take at the current step and generate URBS command.
        
        Step is to be used at every update of the Traveller. URBS commands 
        are generated during traversal following URBS depth-first logic.
    ...
    def _control(self, code: tuple, traveller: Traveller) -> None:
        ...
        code : tuple
            A coded tuple with:
            [0] - The command code (1=RAIN, 2=ADD RAIN, 3=STORE, 4=GET, 5=ROUTE, 7=PRINT)
        ...
        try:
            if command_code == 1:  # RAIN - Start a branch at headwater
                basin = traveller._catchment._vertices[pos]
                reach = traveller.getReach(pos)
                subcatchment_index = basin.index if hasattr(basin, 'index') else pos
                length_km = reach.length() / 1000
                command = f"RAIN #{subcatchment_index} L={length_km:.3f}"
                
                # Add slope parameter if available (slope in m/m for URBS)
                if hasattr(reach, 'slope') and reach.slope is not None:
                    command += f" Sc={reach.slope:.6f}"
                
                self._commandVector.append(command)
                
            elif command_code == 2:  # ADD RAIN - Add subcatchment inflow and route
                basin = traveller._catchment._vertices[pos]
                reach = traveller.getReach(pos)
                subcatchment_index = basin.index if hasattr(basin, 'index') else pos
                length_km = reach.length() / 1000
                ...
                reach = traveller.getReach(pos)
                subcatchment_index = pos  # Use position as subcatchment reference
                length_km = reach.length() / 1000
...
class UrbsCatWriter():
    ...
    def build_cat_file(self, traveller: Traveller) -> str:
        """
        ...
        """
        # URBS .cat file has headers: Index,Name,Area,Imperviousness,IL,CL
        csv_output = StringIO()
        ...
        # Extract subcatchment data from traversal
        subcatchments = self._extract_subcatchments(traveller)
        
        # Write data rows
        for i, subcatchment in enumerate(subcatchments, 1):
            basin = subcatchment['basin']
            name = basin.name if hasattr(basin, 'name') else f"Sub_{i}"
            ...
            cl = basin.cl if hasattr(basin, 'cl') else 0.0  # Continuing Loss
            
            writer.writerow([i, name, area, imperviousness, il, cl])
